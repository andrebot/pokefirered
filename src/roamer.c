...
void CreateRoamerMonInstance(void)
{
    ...
    // Vanilla Rev 1 bug: the roamer's status field is u8, but SetMonData expects a u32*, so it
    // was reading the status field plus the following 3 bytes (cool, beauty, cute) as the status
    // value. Fixed by copying into a proper u32 local first.
